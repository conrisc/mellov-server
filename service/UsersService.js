		...
		function insertNote(db) {
			...
			collection.insertOne(noteItem, function(err, r) {
				if (err) {
					...
					resolve([r.insertedId.toHexString()]);
				}
			});
		}
	});
				...
				collection.findOneAndUpdate(
					...
					.then(result => {
						if (result.ok === 1) {
							const updatedSong = {
								...result.value,
								...{ title: songItem.title, url: songItem.url, tags: correctTags }
						...
						else throw 'result.ok is not equal 1';
					})
					.catch(error => {
